...
Display( )
{
	...
	//float twist = Time * 2 * M_PI;
	Pattern.Use( );
	Pattern.SetUniformVariable( "uTwist", twist.GetValue( Time ) );

	if( UseXYZ )
		Pattern.SetUniformVariable( "uUseChromaDepth", 1 );
	else
		Pattern.SetUniformVariable( "uUseChromaDepth", 0 );
	glCallList( DL );
	Pattern.UnUse( );
...
InitGraphics( )
{
	...
	twist.Init( );
    twist.AddTimeValue(  0.0,  0.000 );
    twist.AddTimeValue(  2.0,  -0.818 );
    twist.AddTimeValue(  5.0,  0.217 );
    twist.AddTimeValue(  8.0,  0.818 );
    twist.AddTimeValue( 10.0,  0.000 );
